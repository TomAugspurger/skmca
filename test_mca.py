...
@pytest.fixture
def burgundies():
    df = pd.read_csv(HERE.joinpath('data/burgundies_flat.csv'),
                     index_col=0, dtype='category')
    return df


class TestMca:

    def test_abdi_valentin(self, burgundies):
        ...
        # correction
        mca = mymca.MCA(benzecri_correction=True)
        mca.fit(burgundies)

        assert_allclose([0.7004, 0.0123, 0.0003], mca.s_[:3]**2, atol=1e-4)
        true_fs_row = [[0.86, 0.08], [-0.71, -0.16], [-0.92, 0.08],
                       [-0.86, 0.08], [0.92, 0.08], [0.71, -0.16]]
        assert_allclose(true_fs_row, mca.fs_r(N=2), atol=1e-2)
        true_fs_col = [[
            .90, -.90, -.97, .00, .97, -.90, .90, .90, -.90, -.9, .90, -.97,
            .00, .97, -.90, .90, .28, -.28, -.90, .90, -.90, .9, .90, -.90
        ], [
            .00, .00, .18, -.35, .18, .00, .00, .00, .0, .00, .00, .18, -.35,
            .18, .00, .00, .0, .00, .00, .00, .00, .00, .00, .00
        ]]
        ...
        true_cont_c = [[
            58, 58, 44, 0, 44, 58, 58, 58, 58, 58, 58, 44, 0, 44, 58, 58, 6, 6,
            58, 58, 58, 58
        ], [
            0, 0, 83, 333, 83, 0, 0, 0, 0, 0, 0, 83, 333, 83, 0, 0, 0, 0, 0, 0,
            0, 0
        ]]
    ...
    def test_abdi_valentin_without_benzecri(self, burgundies):
        # # ... then without Benzecri correction
        mca = mymca.MCA(benzecri_correction=False)
        mca.fit(burgundies)
        assert_allclose(
            [0.8532, 0.2, 0.1151, 0.0317], (mca.s_**2)[:4], atol=1e-4)
        ...
        assert_allclose(mca.expl_, true_expl_var_i, atol=1e-4)
        # assert_allclose(mca.expl_, true_expl_var_c, atol=1e-4)
        # assert_allclose(mca.expl_, true_expl_var_z, atol=1e-4)
        # assert_allclose(mca.expl_, true_expl_var_c, atol=1e-4)
    ...
    def test_abdi_bera(self):
        ...
        df = pandas.read_table(
            'data/music_color.csv', skiprows=0, index_col=0, sep=',')
        ...
        assert_allclose(
            ...
            atol=1e-3)
        assert_allclose(
            mca_df.c, [.11, .11, .11, .11, .11, .11, .11, .11, .11], atol=1e-2)
        ...
        assert_allclose(
            ...
            atol=1e-3)
        assert_allclose(
            mca_df.cont_r(N=2) * 1000,
            [[0, 56], [31, 25], [53, 27], [1, 144], [2, 21], [87, 77],
             [26, 1], [726, 75], [68, 28], [5, 545]],
            atol=1)
        assert_allclose(
        ...
        # Table 3, page 17
        assert_allclose(
            mca_df.fs_c(N=2), [[-0.541, 0.386], [-.257, .275], [-.291, -.309],
                               [.991, .397], [-.122, -.637], [-.236, .326],
                               [.954, -.089], [-.427, .408], [-.072, -.757]],
            atol=1e-3)
        assert_allclose(
            mca_df.cont_c(N=2) * 1000,
            [[113, 86], [25, 44], [33, 55], [379, 91], [6, 234], [22, 61],
             [351, 5], [70, 96], [2, 330]],
            atol=1)
        assert_allclose(
            mca_df.cos_c(N=2) * 1000,
            [[454, 232], [105, 121], [142, 161], [822, 132], [26, 709],
             [78, 149], [962, 8], [271, 249], [7, 759]],
            atol=1)

        assert_allclose(mca_df.L[:2], [.287, .192], atol=2e-3)
        assert round(mca_df.inertia, 3) == 0.746
    ...
    def test_abdi_williams(self):
        ...
        df = pandas.read_table(
            'data/french_writers.csv', skiprows=0, index_col=0, sep=',')
        mca_df = MCA(df, benzecri=False)

        assert_allclose(mca_df.c, [.2973, .5642, .1385], atol=1e-4)
        assert_allclose(
            mca_df.r, [.0189, .1393, .2522, .3966, .1094, .0835], atol=1e-4)
        ...
        assert_allclose(
            -mca_df.fs_c(N=2).T, [[-0.0489, 0.0973, -0.2914],
                                  [.1115, -0.0367, -0.0901]],
            atol=1e-4)
        ...
        assert_allclose(
            mca_df.cont_c(N=2).T, [[0.0399, 0.2999, 0.6601],
                                   [0.6628, 0.1359, 0.2014]],
            atol=1e-4)
        assert_allclose(
            mca_df.cos_c(N=2).T, [[0.1614, 0.8758, 0.9128],
                                  [0.8386, 0.1242, 0.0872]],
            atol=1e-4)

        assert_allclose(mca_df.dc, [0.0148, 0.0108, 0.0930], atol=1e-4)
        assert_allclose(
            mca_df.dr, [0.0630, 0.0474, 0.0116, 0.0084, 0.0543, 0.0408],
            atol=1e-4)

        # abdi = numpy.array([216, 139, 26]) #
        abdi = pandas.DataFrame([216, 139, 26]).T
        assert_allclose(
            mca_df.fs_r_sup(abdi, 2), [[-0.0908, 0.5852]], atol=1e-4)

        supp = pandas.read_table(
            'data/french_writers_supp.csv', skiprows=0, index_col=0, sep=',')
